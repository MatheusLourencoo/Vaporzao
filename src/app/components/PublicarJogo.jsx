...
export default function PublicarJogo({ isLoggedIn, onRequestLogin }) {
  ...
  function toggleGenero(genero) {
    setForm((prev) => ({
      ...
      generosSelecionados: prev.generosSelecionados.includes(genero)
        ? prev.generosSelecionados.filter((g) => g !== genero)
        : [...prev.generosSelecionados, genero]
    }));
  }
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      generoIds: form.generosSelecionados.map((g) => GENERO_IDS[g])
    };
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Compartilhe seu jogo com a comunidade Vaporzão. Você pode publicar até{" "}
          <span className="text-primary font-semibold">3 jogos</span> na plataforma.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2">
              Desenvolvedora <span className="text-primary">*</span>
            </label>
            <input
              required
              type="text"
              value={form.desenvolvedora}
              onChange={(e) => set("desenvolvedora", e.target.value)}
              placeholder="Ex: Banana Studios"
              className="w-full bg-input border border-border rounded-lg px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-all"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Descreva o jogo, mecânicas, história..."
            rows={4}
            className="w-full bg-input border border-border rounded-lg px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-all resize-none"
          />
        </div>
        ...
        <div>
          <label className="block text-sm font-semibold mb-2">
            URL da Capa <span className="text-primary">*</span>
          </label>
          <input
            required
            type="url"
            value={form.capaUrl}
            onChange={(e) => set("capaUrl", e.target.value)}
        ...
        {/* Preço + Data de Lançamento */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-semibold mb-2">
              Preço (R$) <span className="text-primary">*</span>
            </label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="29.90"
                className="w-full bg-input border border-border rounded-lg pl-12 pr-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary transition-all"
              />
            </div>
          </div>
          <div>
            ...
            <input
              ...
              onChange={(e) => set("lancamento", e.target.value)}
        ...
        <div>
          <label className="block text-sm font-semibold mb-3">
            Gêneros <span className="text-primary">*</span>{" "}
            <span className="text-muted-foreground font-normal">(selecione ao menos 1)</span>
          </label>
          <div className="flex flex-wrap gap-2">
            {GENEROS.map((genero) => {
              const selected = form.generosSelecionados.includes(genero);
              return (
                <button
                  key={genero}
                  type="button"
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 rounded-lg px-4 py-3">
          ...
          <span>
            Você pode publicar no máximo <strong className="text-foreground">3 jogos</strong> na plataforma. Certifique-se de que as informações estão corretas antes de enviar.
          </span>
        </div>
